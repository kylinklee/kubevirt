# KubeVirt v1.8.2 兼容 K8s 1.31 修改说明

## 背景

KubeVirt v1.8.2 官方仅支持 K8s 1.33-1.35。本分支通过源码修改使其可在 K8s 1.31 上编译和运行。
...
kubectl patch kubevirt kubevirt -n kubevirt --type=json \
  -p '[{"op": "add", "path": "/spec/imageTag", "value": "v1.8.2-k8s131"}]'
