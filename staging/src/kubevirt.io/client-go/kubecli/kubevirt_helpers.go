// [K8s 1.31兼容] 从 kubevirt_test_utils.go 拆分出的生产代码。
// 原文件中的 mock 辅助代码与 K8s 1.34 的 generated_mock 绑定，
// 这些 NewMinimal* 工具函数不依赖 mock，独立出来确保正常编译。
...
func NewMinimalCloneWithNS(name, namespace string) *clone.VirtualMachineClone {
	return &clone.VirtualMachineClone{
		...
		ObjectMeta: k8smetav1.ObjectMeta{Name: name, Namespace: namespace},
	}
}
